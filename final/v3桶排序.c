...
int main()
{
	...
	for(int i=0; i<SentencePos; i++)
	{
		...
		else if( list[i] > heap[0] )
		{
			heap[0]=list[i];
			AdjustDown(HeapCount);
		}
	}
...
{
	for(int child=index, int parent=child/2; child>0; child=parent, parent=child/2)
	{
		if(list[child].sum < list[parent].sum)
		{
			Swap( &(list[child]), &(child[parent]) );
		}
		else
		{
			break;
